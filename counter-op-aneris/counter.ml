...
let rec listen_wait skt =
  match receiveFrom skt with Some msg -> msg | None -> listen_wait skt
...
let dict_lookup k db = try Some (List.assoc k db) with Not_found -> None
...
let vect_leq v1 v2 = List.for_all2 (fun a b -> a <= b) v1 v2
...
let rec vect_deserialize s =
  try
    ...
    try
      ...
      let vc =
        vect_deserialize (String.sub s (i + 1) (String.length s - (i + 1)))
      in
      x :: vc
...
let prod_deser deserA deserB s =
  try
    ...
    let s2 = String.sub s (i + 1 + len) (String.length s - (i + 1 + len)) in
    let v1 = deserA s1 in
    let v2 = deserB s2 in
    (v1, v2)
  with _ -> assert false
...
let oper_ser =
  prod_ser (prod_ser (prod_ser string_ser int_ser) vect_serialize) int_ser

let oper_deser =
  prod_deser
    (prod_deser (prod_deser string_deser int_deser) vect_deserialize)
    int_deser

let ack_ser = prod_ser (prod_ser string_ser int_ser) int_ser

let ack_deser = prod_deser (prod_deser string_deser int_deser) int_deser
